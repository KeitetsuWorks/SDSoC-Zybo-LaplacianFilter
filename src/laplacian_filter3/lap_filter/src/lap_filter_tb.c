/**
 * @file        lap_filter_tb.c
 * @brief       Testbench of lap_filter.c
 *              BMPデータをハードウェアとソフトウェアで、ラプラシアン・フィルタを掛けて、それを比較する
 *              m_axi offset=slave version
 * @author      marsee
 * @date        2015/08/26
 * @see         http://marsee101.blog19.fc2.com/blog-entry-3688.html
 */
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            if (y == 0 || y == height - 1) { // 縦の境界の時の値は0とする
                ...
                sl = y % 3;         // 2番目のライン
                tl = (y + 1) % 3;   // 3番目のライン
                lap_fil_val = laplacian_fil_soft(line_buf[fl][x - 1], line_buf[fl][x], line_buf[fl][x + 1],
                                                 line_buf[sl][x - 1], line_buf[sl][x], line_buf[sl][x + 1],
                                                 line_buf[tl][x - 1], line_buf[tl][x], line_buf[tl][x + 1]);
            }
            // ラプラシアンフィルタ・データの書き込み
